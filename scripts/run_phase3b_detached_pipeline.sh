...
export PHASE3B_PHYSICAL_GPU=1
cd "$ROOT"

mkdir -p "$OUT/logs" "$OUT/audit/preflight" "$OUT/reports"
echo "PIPELINE_START $(date -u +%FT%TZ) pid=$$ physical_gpu=1" >"$OUT/logs/detached_pipeline_state.log"
...
stage B1_PREFLIGHT
"$DEEPSPEED" --num_gpus=1 --master_port 29531 scripts/phase3b_train.py \
...
"$DEEPSPEED" --num_gpus=1 --master_port 29532 scripts/phase3b_train.py \
  --config configs/phase3b_b1_generated_replay.yaml --workers 8 \
  >"$OUT/logs/b1_formal_training.log" 2>&1

stage B0_PREFLIGHT
"$DEEPSPEED" --num_gpus=1 --master_port 29533 scripts/phase3b_train.py \
...
"$DEEPSPEED" --num_gpus=1 --master_port 29534 scripts/phase3b_train.py \
